...
def integrity_check():
    ...
    integrity_check, error_address =  validate_blockchain(blocks)
    
    if integrity_check:
        message = "Cơ sở dữ liệu toàn vẹn!"
    else:
        message = f"Cơ sở dữ liệu không toàn vẹn, lỗi xuất hiện tại vị trí block {error_address}"

    return render_template('integrity_check.html', blocks = blocks, result = message)
